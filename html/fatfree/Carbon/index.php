...
$f3->route('POST /simpleform',
  function($f3) {
	...
	$formdata["food"] = $f3->get('POST.food');			// whatever was called "name" on the form
	$formdata["frequency"] = $f3->get('POST.frequency');		// whatever was called "diet" on the form
		
  	$controller = new SimpleController;
    $result = $controller->getResults($formdata["food"], $formdata["frequency"]);
  	
	$f3->set('viewResult',$result);		// set info in F3 variable for access in response template
      
    
//print_r($result);
//   echo json_encode(true);
echo template::instance()->render('response.html');
  }
);
...
//  function($f3) {
//	$formdata = array();			// array to pass on the entered data in
//	$formdata["food"] = $f3->get('GET.food');			// whatever was called "name" on the form
//	$formdata["frequency"] = $f3->get('GET.frequency');		// whatever was called "diet" on the form
//		
//  	$controller = new SimpleController;
//    $result = $controller->getResults($formdata["food"], $formdata["frequency"]);
//  	
